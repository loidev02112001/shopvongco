...
import {
  Bell, HelpCircle, Heart, User, ShoppingBag,
  Facebook, Instagram, Youtube, Globe, Search as SearchIcon,
...
function NewsletterForm() {
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    toast.success("Đăng ký nhận tin thành công!", { description: `Cảm ơn ${email} đã đăng ký. Mã ưu đãi sẽ được gửi qua email.` });
    setEmail("");
  };
  return (
    <form onSubmit={onSubmit} className="flex group transition-all hover:shadow-md rounded">
      ...
      />
      <button type="submit" className="bg-brand text-brand-foreground px-4 rounded-r text-sm font-semibold hover:brightness-110 hover:bg-brand/90 transition">
        Sign up
      </button>
    </form>
  );
      ...
      <div className="max-w-7xl mx-auto px-6 py-4 flex items-center gap-6">
        ...
        <div className="flex items-center gap-5 text-brand">
          <Link to="/yeu-thich" aria-label="Yêu thích" className="relative">
            ...
            {wishCount > 0 && (
              <span className="absolute -top-2 -right-2 bg-price text-white text-[10px] rounded-full w-4 h-4 flex items-center justify-center">{wishCount}</span>
            )}
          </Link>
          <Link to="/gio-hang" aria-label="Giỏ hàng" className="relative">
            <ShoppingBag className="w-5 h-5" />
            {cartCount > 0 && (
              <span className="absolute -top-2 -right-2 bg-price text-white text-[10px] rounded-full w-4 h-4 flex items-center justify-center">{cartCount}</span>
            )}
          </Link>
          <UserMenu />
        </div>
        <div className="flex flex-col gap-1.5 text-xs">
          <Link to="/thong-bao" className="flex items-center gap-1.5 border border-brand/40 rounded-full px-3 py-1 text-foreground/70 hover:bg-brand-soft transition">
            <Bell className="w-3.5 h-3.5" /> Thông báo
          </Link>
          <Link to="/ho-tro" className="flex items-center gap-1.5 border border-brand/40 rounded-full px-3 py-1 text-foreground/70 hover:bg-brand-soft transition">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-56 bg-white border border-border rounded-xl shadow-lg py-2 z-50 animate-in fade-in slide-in-from-top-1 duration-150">
          <div className="px-4 py-3 border-b border-border">
            <div className="font-semibold text-sm text-foreground truncate">{currentUser.fullName}</div>
...
function SearchForm() {
  ...
  const suggestions = useMemo(() => {
    ...
    const list = needle
      ? products.filter((p) => p.name.toLowerCase().includes(needle) || p.shortName.toLowerCase().includes(needle))
      : products;
  ...
  return (
    <div ref={wrapRef} className="relative">
      <form
        onSubmit={(e) => { e.preventDefault(); go(undefined, q); }}
      ...
      >
        ...
        />
        <button type="submit" className="bg-brand text-brand-foreground px-8 text-sm font-medium hover:opacity-90 transition">
          Search
        </button>
      </form>
      ...
      {open && suggestions.length > 0 && (
        <div className="absolute z-30 left-0 right-0 mt-1 bg-white border border-border rounded-md shadow-lg max-h-96 overflow-auto">
          {!q.trim() && (
            <div className="px-4 py-2 text-[11px] uppercase tracking-wide text-muted-foreground border-b">Gợi ý cho bạn</div>
          )}
          {suggestions.map((p) => (
            <button
              key={p.slug}
...
export function ProductCard({ slug, img, name, price }: { slug?: string; img: string; name: string; price: string }) {
...
function ProductCardInner({ slug, img, name, price }: { slug?: string; img: string; name: string; price: string }) {
  ...
  const inner = (
    <div className="bg-white rounded-md overflow-hidden group cursor-pointer">
      <div className="relative overflow-hidden">
        ...
        >
          <Heart className={`w-6 h-6 ${liked ? "fill-[#7CC5CC] text-[#7CC5CC]" : ""}`} strokeWidth={2} />
        </button>
        <TryOnBadge slug={slug} />
      </div>
      <div className="p-4 text-left">
        <h3 className="t-product text-foreground/85 leading-snug line-clamp-2">{name}</h3>
        <p className="mt-2 text-price t-price">{price}</p>
      </div>
  ...
  return (
    <footer className="mt-20" style={{ backgroundColor: "#C8C8C8" }}>
      <div className="max-w-7xl mx-auto px-6 py-10 grid grid-cols-2 md:grid-cols-4 gap-8 text-sm">
        {/* Chính Sách */}
        <div>
          <h4 className="font-bold mb-4 text-sm" style={{ color: "#000000" }}>Chính Sách</h4>
          <ul className="space-y-2" style={{ color: "#000000" }}>
            <li><Link to="/dich-vu" search={{ muc: "bao-mat" }} className="hover:text-gray-700 transition">Chính sách bảo mật</Link></li>
            <li><Link to="/dich-vu" search={{ muc: "bao-hanh" }} className="hover:text-gray-700 transition">Chính sách bảo hành</Link></li>
            <li><Link to="/dich-vu" search={{ muc: "doi-tra" }} className="hover:text-gray-700 transition">Chính sách đổi trả</Link></li>
            <li><Link to="/dich-vu" search={{ muc: "van-chuyen" }} className="hover:text-gray-700 transition">Chính sách vận chuyển</Link></li>
          </ul>
        </div>
        {/* Hỗ trợ */}
        <div>
          <h4 className="font-bold mb-4 text-sm" style={{ color: "#000000" }}>Hỗ trợ</h4>
          <ul className="space-y-2" style={{ color: "#000000" }}>
            <li><Link to="/dieu-khoan-dich-vu" className="hover:text-gray-700 transition">Điều khoản dịch vụ</Link></li>
            <li><Link to="/huong-dan-mua-hang" className="hover:text-gray-700 transition">Hướng dẫn mua hàng</Link></li>
            <li><Link to="/huong-dan-thanh-toan" className="hover:text-gray-700 transition">Hướng dẫn thanh toán</Link></li>
          </ul>
        </div>
        {/* Thông tin liên hệ */}
        <div>
          <h4 className="font-bold mb-4 text-sm" style={{ color: "#000000" }}>Thông tin liên hệ</h4>
          <ul className="space-y-2" style={{ color: "#000000" }}>
            <li>Đại học FPT, Khu CNC Hòa Lạc,<br />Thạch Thất, Hà Nội</li>
            <li>0901 234 567</li>
        ...
        {/* Đăng Kí Nhận Tin */}
        <div>
          <h4 className="font-bold mb-4 text-sm" style={{ color: "#000000" }}>Đăng Kí Nhận Tin</h4>
          <NewsletterForm />
          <div className="flex gap-2 mt-4">
            ...
            <a href="https://lunajewel.vn" target="_blank" rel="noopener noreferrer" aria-label="Website"
              className="w-8 h-8 rounded-full flex items-center justify-center hover:opacity-80 transition"
      ...
      <div className="py-3 text-center text-xs" style={{ backgroundColor: "#6B6B6B", color: "#fff" }}>
